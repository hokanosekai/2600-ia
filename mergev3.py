...
def balance_classes(X, y, samples_per_class=1500, random_state=42):
    """Équilibre les classes en utilisant over-sampling et under-sampling."""
    if not HAVE_IMBLEARN:
        print("⚠️ imbalanced-learn non disponible, impossible d'équilibrer les classes.")
        return X, y
    
    print(f"➡️ Équilibrage appliqué à {samples_per_class} échantillons par classe.")
    print("Avant:")
    print(y.value_counts())
    
    # Stratégie pour avoir exactement samples_per_class par classe
    sampling_strategy = {cls: samples_per_class for cls in y.unique()}
    
    # Utiliser SMOTE + RandomUnderSampler pour obtenir exactement le nombre souhaité
...
def create_safe_features(df: pd.DataFrame) -> pd.DataFrame:
    """Crée des features dérivées sans générer de warnings."""
    df = df.copy()
    
    # Ratios et features dérivées - version safe
    if 'Total Fwd Packets' in df.columns and 'Total Backward Packets' in df.columns:
        df['Total_Packets'] = df['Total Fwd Packets'] + df['Total Backward Packets']
        # Éviter la division par zéro
        bwd_packets_safe = df['Total Backward Packets'].replace(0, 1)
        df['Fwd_Bwd_Ratio'] = df['Total Fwd Packets'] / bwd_packets_safe
        
        total_packets_safe = df['Total_Packets'].replace(0, 1)
        df['Fwd_Packet_Ratio'] = df['Total Fwd Packets'] / total_packets_safe
    ...
    if 'Fwd Packets Length Total' in df.columns and 'Bwd Packets Length Total' in df.columns:
        ...
        total_packets_safe = df['Total_Packets'].replace(0, 1)
        df['Bytes_per_Packet'] = df['Total_Bytes'] / total_packets_safe
        
        # Éviter la division par zéro
        bwd_bytes_safe = df['Bwd Packets Length Total'].replace(0, 1)
        df['Fwd_Bwd_Bytes_Ratio'] = df['Fwd Packets Length Total'] / bwd_bytes_safe
    
    # Features de temporalité - version safe
    if 'Flow Duration' in df.columns:
        # Éviter les valeurs négatives ou nulles
        flow_duration_safe = df['Flow Duration'].clip(lower=1)
        df['Bytes_per_Second'] = df['Total_Bytes'] / flow_duration_safe
        df['Packets_per_Second'] = df['Total_Packets'] / flow_duration_safe
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=1500,
        help="Nombre d'échantillons par classe pour l'équilibrage (0 pour désactiver).",
    )
    p.add_argument(
        "--smote-k-neighbors",
        type=int,
    ...
    print("=== ENTRAÎNEMENT AVEC ÉQUILIBRAGE DES CLASSES ===")
    ...
    print(f"Colonne cible '{target_col}' identifiée. Distribution initiale:")
    print(y.value_counts())

    # Feature engineering safe
    if args.feature_engineering:
        print("\n--- Feature Engineering ---")
        X = create_safe_features(X)
    ...
    if args.balance_classes > 0:
        print("\n--- Équilibrage des Classes ---")
        X, y = balance_classes(X, y, samples_per_class=args.balance_classes, random_state=args.seed)
    ...
    def make_training_pipeline():
        """Construit la pipeline d'entraînement."""
        # Après équilibrage manuel, pas besoin de resampling supplémentaire
    ...
    if args.testsample > 0:
        ...
        backup_name = f"{timestamp}-{args.model}-balanced-{test_accuracy:.4f}.skio"
    else:
        out_file_name = f"student_model.skio"
        backup_name = f"{timestamp}-{args.model}-balanced-final.skio"
