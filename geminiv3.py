...
def create_balanced_training_set(original_parquet_path, 
                                 output_path="Oversampling_1500.parquet", 
    ...
    # Supprimer l'ancienne colonne 'Label' si elle existe (comme dans votre 1er script)
    ...
    for cls in classes:
        print(f"Échantillonnage de {samples_per_class} échantillons pour la classe '{cls}'...")
        class_df = df_full[df_full['ClassLabel'] == cls]
        
        # Gérer les classes qui ont MOINS de 1500 échantillons
        if len(class_df) < samples_per_class:
            print(f"  Alerte : La classe '{cls}' n'a que {len(class_df)} échantillons.")
            print("  Utilisation de tous les échantillons disponibles (avec remplacement) pour atteindre 1500.")
            # 'replace=True' permet de sur-échantillonner pour atteindre 1500
            sampled_dfs.append(class_df.sample(n=samples_per_class, random_state=random_state, replace=True))
        else:
            # 'replace=False' est la valeur par défaut
            sampled_dfs.append(class_df.sample(n=samples_per_class, random_state=random_state))
            
    print("Combinaison et mélange des échantillons...")
    df_balanced = pd.concat(sampled_dfs)
    
    # Mélange final pour que les classes ne soient pas groupées
...
if __name__ == "__main__":
    ...
    # (D'après votre premier script, il est dans ce dossier)
    ORIGINAL_DATA_PATH = 'Training.parquet'
    NEW_TRAINING_PATH = 'Balanced.parquet'
    ...
    print(f"Chargement de {NEW_TRAINING_PATH} (version équilibrée)...")
    try:
    ...
    pipeline = Pipeline([
        ...
        ('classifier', RandomForestClassifier(
            ...
            max_depth=20,
            min_samples_leaf=5,  
            # 'class_weight' n'est plus 'balanced' car nous avons équilibré manuellement
            # Cela peut même améliorer la performance
    ...
    # 'stratify=y' est toujours une bonne idée, même si c'est équilibré
    ...
    print("\nDébut de l'entraînement final (sur 100% des données équilibrées)...")
    pipeline.fit(X, y)
