...
def create_balanced_training_set(original_parquet_path, 
                                 ...
                                 samples_per_class=1500, #<- Valeur par défaut
                                 random_state=42):
    """
...
if __name__ == "__main__":
    ...
    # J'utilise un nouveau nom pour ce test
    NEW_TRAINING_PATH = 'Oversampling_1500.parquet' 
    ...
    success = create_balanced_training_set(
        ...
        samples_per_class=1500  # <--- C'EST LE SEUL CHANGEMENT
    )
    ...
    print(f"Chargement de {NEW_TRAINING_PATH} (version 1.5k/classe)...")
    try:
    ...
    print("\nDébut de l'entraînement final (sur 100% des données 1.5k/classe)...")
    pipeline.fit(X, y)
